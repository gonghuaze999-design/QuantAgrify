...
interface WelcomeHubProps {
  onNavigate: (view: 'dataSource' | 'algorithm' | 'cockpit' | 'api') => void;
}
...
export const WelcomeHub: React.FC<WelcomeHubProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-white font-['Space_Grotesk'] overflow-hidden flex flex-col relative selection:bg-[#0d59f2]/30">
      ...
      {/* Navigation Bar - Updated to Text Links for consistency */}
      <nav className="h-16 border-b border-white/10 bg-[#0a0e17]/80 backdrop-blur-2xl flex items-center justify-between px-8 z-[100] relative">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-3 cursor-pointer group">
            <div className="bg-[#0d59f2] p-2 rounded shadow-[0_0_20px_rgba(13,89,242,0.3)] group-hover:scale-105 transition-transform">
              <span className="material-symbols-outlined text-white text-2xl block">agriculture</span>
            </div>
            <div className="flex flex-col leading-none">
              <span className="text-2xl font-bold text-white tracking-tighter uppercase">QuantAgrify</span>
              <span className="text-[8px] font-black text-[#90a4cb] tracking-[0.3em] uppercase mt-0.5 opacity-60">System v4.0 Alpha</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-10">
            {navItems.map((item) => (
              <button 
                ...
                className="text-[10px] font-black uppercase tracking-[0.3em] text-[#90a4cb] hover:text-white transition-all relative group h-16"
              >
                {item.label}
                <div className="absolute bottom-0 left-0 w-0 h-[2px] bg-[#0d59f2] transition-all group-hover:w-full shadow-[0_0_8px_#0d59f2]"></div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="hidden md:flex items-center gap-4 bg-white/[0.03] px-5 py-2 rounded-full border border-white/5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-[#94a3b8] uppercase tracking-widest font-mono">Engine: 99.9%</span>
            </div>
          ...
          <div className="flex items-center gap-3 pl-4 border-l border-white/10">
            ...
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-[#0d59f2] to-[#00f2ff] p-[1px] shadow-lg shadow-[#0d59f2]/20">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-8 relative overflow-hidden">
        ...
        <div className="w-full max-w-5xl mb-24 relative px-12 pt-16">
          {/* Header text group - Adjusted position and Z-index to prevent overlap */}
